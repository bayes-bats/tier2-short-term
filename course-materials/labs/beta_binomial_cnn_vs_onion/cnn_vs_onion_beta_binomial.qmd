...
![Front page of the Onion. Photo: Casey Bisson, via Flickr](the_onion.jpg){fig-alt="Front page of Onion Newspaper: Headline reads Wikipedia Celebrates 750 Years of American Independence"}
...
unsure <- plot_beta(1, 1) +
  labs(title = "Unsure")

pessimistic <- plot_beta(5, 10) +
  labs(title = "Pessimistic")

gridExtra::grid.arrange(optimistic, unsure, pessimistic, ncol = 3)
```
...
```{r summarize Colby 2010 optimist}
...
```{r summarize Colby 2010 pessimist}
...
```{r summarize Colby 2010 very optimistic}
summarize_beta_binomial(alpha = 140, beta = 10, y = 45, n = 150)
```

```{r fig.align = "center", fig.height = 5}
plot_beta_binomial(alpha = 140, beta = 10, y = 45, n = 150)
```
...
unsure <- plot_beta_binomial(1, 1, y = 45, n = 150) +
  labs(title = "Unsure")
...
gridExtra::grid.arrange(optimist, unsure, pessimist, very_optimistic, ncol = 2)
...
gridExtra::grid.arrange(optimist, unsure, pessimist, very_optimistic, your_plot, ncol = 2)
...
- Look back at the summary statistics that you calculated for the distributions. Choose one of the posteriors "Optimistic, Pessimistic, Unsure, Very Optimistic" and compare it with your own. What do each of the summary statistics (mean, mode, standard deviation) tell you about the probability of success after seeing the data?
