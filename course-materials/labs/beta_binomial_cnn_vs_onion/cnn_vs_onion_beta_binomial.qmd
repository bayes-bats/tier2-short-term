...
Tip: You can break this down into how many questions out of 15 you expect to get correct: Beta(number_correct, number_wrong).

Replace `number_correct` and `number_wrong` with your predictions.


```{webr-r}

my_prediction <- plot_beta(number_correct, number_wrong) +
  labs(title = "My Prediction")

my_prediction
```

Which guesser is your prior closest to or most different from?
...
- Our data is a data frame called `cnn_onion`. Select the data from the year the quiz began.

- Based on the observed data, we will update the posterior for our three guessers and our own prior. 

- Next, we calculate the summary statistics for the prior and posterior for all four.
...
- We plot the prior, likelihood, and the posterior for all four.

- We examine the effect of different priors on the posterior.
...
Reading in the data from the google form (under construction)
...
summarize_beta_binomial(14, 1, y = 45, n = 150)
```

```{r fig.align = "center", fig.height = 5}
plot_beta_binomial(14, 1, y = 45, n = 150)
```

### The Unpredictable Guesser

```{r summarize Colby 2010 unpredictable}
summarize_beta_binomial(1, 1, y = 45, n = 150)
```

```{r fig.align = "center", fig.height = 5}
plot_beta_binomial(1, 1, y = 45, n = 150)
```

### The Poor Guesser

```{r summarize Colby 2010 poor}
summarize_beta_binomial(4, 11, y = 45, n = 150)
```

```{r fig.align = "center", fig.height = 5}
plot_beta_binomial(4, 11, y = 45, n = 150)
```
...
```{webr-r}
summarize_beta_binomial(___, ___, y = 45, n = 150)
```

```{webr-r}
plot_beta_binomial(___, ___, y = 45, n = 150)
```
...
Fill in the gap alpha and beta shape parameters with your guess:

```{webr-r}
library(patchwork)

your_plot <- plot_beta_binomial(1, 1, y = 45, n = 150) +
